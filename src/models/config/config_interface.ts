import { AI_Config, createAI_Config } from './AI/AI_Config_interface.ts';
import { AI_Entity, createAI_Entity } from './AI/AI_Entity_interface.ts';
...
export function getConfigReturnAIConfig(obj: AI_Config): AI_Config {
  ...
  return createAI_Config(
    obj.architectors.map((architector: AI_Entity) => createAI_Entity(
      architector.name,
    ...
    obj.workers.map((worker: AI_Entity) => createAI_Entity(
      worker.name,
      worker.description,
    ...
    obj.acceptance.map((acceptance: AI_Entity) => createAI_Entity(
      acceptance.name,
